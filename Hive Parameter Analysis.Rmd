...
bee_param_df <- list.files(path="hive_data/m2_eating/", full.names = TRUE) %>% 
                lapply(read_csv, show_col_types = FALSE) %>% 
                bind_rows

write.csv(bee_param_df,"bee_data_isaac_m2.csv",row.names = FALSE)
...
#m2B_bee_param <- read.csv("bee_data_isaac_m2_BUGGED.csv") %>% mutate(type = "M2B") %>% select(-trial_n)

bee_param_df <- rbind(m1_bee_param,m2_bee_param) %>% rbind(.,rand_bee_param) #%>% rbind(.,m2B_bee_param)
```
