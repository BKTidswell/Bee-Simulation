...
bee_param_df <- list.files(path="hive_data/m2_empty/", full.names = TRUE) %>% 
                lapply(read_csv, show_col_types = FALSE) %>% 
                bind_rows

write.csv(bee_param_df,"bee_data_isaac_m2_empty.csv",row.names = FALSE)
...
bee_param_df <- rbind(m1_bee_param,m2_bee_param) %>% rbind(.,rand_bee_param) %>% rbind(.,m2E_bee_param)
```
