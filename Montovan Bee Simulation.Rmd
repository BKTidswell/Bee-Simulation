...
get_brood_density <- function(x_ind,y_ind,hive_data){
  xLoc <- (hexdat_centers %>% filter(Xind == y_ind, Yind == x_ind))$x
  yLoc <- (hexdat_centers %>% filter(Xind == y_ind, Yind == x_ind))$y
      ...
      layer2_val <- ifelse(set_hive[x,y,1] == BROOD, sample(1:MAX_BROOD, 1),
                          ifelse(set_hive[x,y,1] == HONEY, sample(1:MAX_HONEY,1),
                                 ifelse(set_hive[x,y,1] == POLLEN, sample(1:MAX_POLLEN,1),
                                        0)))
      layer3_val <- ifelse(set_hive[x,y,1] == BROOD, sample(1:24, 1), 0)
      
      set_hive[x,y,2] <- layer2_val
      set_hive[x,y,3] <- layer3_val
    }
  }
...
collect_products <- function(hive_data,product,product_max){
  for(i in 1:N_ATTEMPTS){
    rand_x <- sample(1:MAX_ROWS,1)
    rand_y <- sample(1:MAX_COLS,1)
    
    if(hive_data[rand_x,rand_y,1] == product && hive_data[rand_x,rand_y,2] < product_max){
      hive_data[rand_x,rand_y,2] <- hive_data[rand_x,rand_y,2] + 1
      break
    } else if(hive_data[rand_x,rand_y,1] == EMPTY){
      hive_data[rand_x,rand_y,1] <- product
      hive_data[rand_x,rand_y,2] <- 1
      break
    }
  }
  return(hive_data)
...
eat_products <- function(hive_data,product){
  for(i in 1:N_ATTEMPTS){
    ...
    if(hive_data[rand_x,rand_y,1] == product){
      hive_data[rand_x,rand_y,2] <- hive_data[rand_x,rand_y,2] - min(hive_data[rand_x,rand_y,2],
                                                                     1 + get_brood_density(rand_x,rand_y,hive_data)*(k-1))
      if(hive_data[rand_x,rand_y,2] == 0){
        hive_data[rand_x,rand_y,1] <- EMPTY
      }
      break
    }
  }
...
hatch_brood <- function(hive_data,hour){
  hatch_x <- data.frame(which(hive_data[,,1] == BROOD & hive_data[,,2] >= MAX_BROOD & hive_data[,,3] == hour,arr.ind = TRUE))$row
  hatch_y <- data.frame(which(hive_data[,,1] == BROOD & hive_data[,,2] >= MAX_BROOD & hive_data[,,3] == hour,arr.ind = TRUE))$col
  
  #Hatch all brood that hatch this hour
  for(i in 1:length(hatch_x)){
    hive_data[hatch_x[i],hatch_y[i],1] <- EMPTY
    hive_data[hatch_x[i],hatch_y[i],2] <- 0
    hive_data[hatch_x[i],hatch_y[i],3] <- 0
  }
