...
hive_matrix_to_graph <- function(hive_data,queen){
  ...
  # 
  # queen_locs[queen$cur_Y,queen$cur_X] <- 1
  # 
  ...
  g <- ggplot() + 
        ...
        # geom_point(new_queen_locs, mapping = aes(x,y), color = "black", size = 2) +
        # geom_point(new_queen_locs, mapping = aes(x,y), color = "yellow", size = 1) +
        coord_fixed() +
        theme_classic()
...
queen <- Queen$new(ceiling(MAX_COLS/2), ceiling(MAX_ROWS/2))
...
av::av_encode_video(paste0(productPath,list.files(productPath, '*.png')), framerate = 10,
                    output = paste0("hive_videos/bee_video_D",DAYS,
                                    "_H",HONEY_BY_HOUR,
                                    "_P",POLLEN_BY_HOUR,
                                    "_Q",QUEEN_CELLS_PER_HOUR,
                                    "_HE",HONEY_EATEN_PER_HOUR,
                                    "_PE",POLLEN_EATEN_PER_HOUR,
                                    ".mp4"))
```
