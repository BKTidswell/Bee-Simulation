...
hex_move <- function(cur_X,cur_Y,prev_X,prev_Y){
  ...
  legal_hexes <- legal_hexes %>% filter(x > 0) %>% filter(y > 0) %>%
                                 filter(x <= MAX_COLS) %>% filter(y <= MAX_ROWS) %>%
...
hexes_dist_to_nearest_contents <- function(xHex,yHex,content,hive_data){
  ...
  hive_df <- as.data.frame(as.table(hive[,,1:2])) %>% pivot_wider(names_from = Var3, values_from = Freq)
  colnames(hive_df) <- c("y","x","contents","amount")
...
make_set_hive <- function(){
  ...
  all_to_pollen <- hexes_in_rad(MAX_X/2+0.75,MAX_Y/2+1,BROOD_RADIUS+POLLEN_WIDTH)
  ...
  all_to_brood <- hexes_in_rad(MAX_X/2+0.75,MAX_Y/2+1,BROOD_RADIUS)
...
Queen <- R6Class("Queen",
                 public = list(
                   ...
                   interact_with_comb = function(current_hive){
                     curr_fill <- current_hive[self$cur_Y,self$cur_X,1]
                     curr_val <- current_hive[self$cur_Y,self$cur_X,2]
                     
                     #If you end up on an empty hive
                     if(curr_fill == EMPTY && 
                        hexes_dist_to_nearest_contents(self$cur_X,self$cur_Y,BROOD,current_hive) <= BROOD_RADIUS){
                       current_hive[self$cur_Y,self$cur_X,1] <- BROOD
                       current_hive[self$cur_Y,self$cur_X,2] <- 0
                       current_hive[self$cur_Y,self$cur_X,3] <- sample(1:24, 1)
                     }
                     return(current_hive)
                   },
                   buzz = function(){
                     cat(paste0("Hello, I am a queen!\n",
...
New_Queen <- R6Class("New Queen",
                 public = list(
                   ...
                   move = function(){
                     angle_move <- runif(1,-pi,pi)
                     
                     next_X_ind <- self$cur_X_ind
                     next_Y_ind <- self$cur_Y_ind
                     dist <- 0.1
                     
                     while(next_X_ind == self$cur_X_ind && next_Y_ind == self$cur_Y_ind){
                       next_X <- self$cur_X + dist*cos(angle_move)
                       next_Y <- self$cur_Y + dist*sin(angle_move)
                       near_hex <- nearest_hex(next_X,next_Y)
                       next_X_ind <- max(min(near_hex[[1]],MAX_COLS),0)
                       next_Y_ind <- max(min(near_hex[[2]],MAX_ROWS),0)
                     ...
                     print(angle_move)
                     print(c(self$cur_X,self$cur_Y))
                     print(c(self$cur_X_ind,self$cur_Y_ind))
                     self$cur_X <- next_X
                     self$cur_Y <- next_Y
                     self$cur_X_ind <- next_X_ind
                     self$cur_Y_ind <- next_Y_ind
                     print(c(self$cur_X,self$cur_Y))
                     print(c(self$cur_X_ind,self$cur_Y_ind))
                   },
                   lay_brood = function(current_hive){
                     ...
                     distance_to_brood <- round(hexes_dist_to_nearest_contents(self$cur_X_ind,self$cur_Y_ind,BROOD,current_hive),2)
                     
                     if(distance_to_brood == 1.73){
                       print("COME AND GET ME SOMETHING IS SO BAD AND WRONG")
                     }
                     ...
                     print(c(self$cur_X,self$cur_Y))
                     print(curr_fill)
                     print(distance_to_brood)
                     print(curr_fill == EMPTY)
                     print(distance_to_brood <= BROOD_RADIUS)
                     print(curr_fill == EMPTY && 
                        distance_to_brood <= BROOD_RADIUS)
...
hexes_dist_to_nearest_contents(20,37,BROOD,hive) <= BROOD_RADIUS

[1] "Moving"
[1] 1
[1] -2.312226
[1]  0.4793277 14.1491788
[1]  1 59
[1] -0.6012213 12.9691730
[1]  1 61
[1] "COME AND GET ME SOMETHING IS SO BAD AND WRONG"
[1] "Laying??"
[1]  1 61
[1] -0.6012213 12.9691730
[1] 0
...
To find the nearest hex to a point
```{r Nearest Hex Index to Point Redo}
nearest_hex <- function(xLoc,yLoc){
  xLoc <- -0.6012213
  yLoc <- 12.9691730
  hex_dist <- hexdat_centers %>% mutate(dist = sqrt((xLoc-x)**2+(yLoc-y)**2)) %>% arrange(dist)
...
while(next_X_ind == cur_X_ind && next_Y_ind == cur_Y_ind){
  next_X <- cur_X + dist*cos(angle_move)
  next_Y <- cur_Y + dist*sin(angle_move)
  near_hex <- nearest_hex(next_X,next_Y)
  print(c(next_X,next_Y))
  next_X_ind <- max(min(near_hex[[1]],MAX_COLS),0)
  next_Y_ind <- max(min(near_hex[[2]],MAX_ROWS),0)
  print(c(next_X_ind,next_Y_ind))
...
for(trial in 1:N_TRIALS){
  ...
  queen <- New_Queen$new(median(hexdat_centers$x),median(hexdat_centers$y))
        ...
        if(o == 1){
          ...
          if(h <= 12){
            ...
            for(i in 1:QUEEN_CELLS_PER_HOUR){
              #Each one takes about 0.015
              #Moving takes 0.005
              #Laying takes about 0.01
...
Turn images into video (Only if needed here)
```{r Images to Video}
# productPath <- ('/Users/Ben/Desktop/Bee-Simulation/hive_plots/')
# av::av_encode_video(paste0(productPath,list.files(productPath, '*.png')), framerate = 10,
#                     output = paste0("hive_videos/bee_video_D",
