...
```{r Run Simulation, warning = FALsSE}
N_TRIALS <- 10
N_DAYS <- 20
...
for(trial in 1:N_TRIALS){
      ...
      for(o in order){
        ...
        if(o == 1){
          ...
          if(h <= 12){
            ...
            # to keep it balanced
            while(honey_eaten < HONEY_EATEN_PER_HOUR && honey_eat_attempts < HONEY_EATEN_PER_HOUR){
            ...
            while(pollen_eaten < POLLEN_EATEN_PER_HOUR && pollen_eat_attempts < POLLEN_EATEN_PER_HOUR){
  ...
  ggsave(paste0("hive_plots/hiveplot_",str_pad((d-1)*24+h+1, 5, pad = "0"),
                    "_D",str_pad(d, 3, pad = "0"),
                    "_T",str_pad(trial, 3, pad = "0"),".png"),
             graph_trends(Count_Contents),width=7, height=7)
  ...
  av::av_encode_video(paste0(productPath,list.files(productPath, paste0("*_T",str_pad(trial, 3, pad = "0"),
                                                                        '.png'))), framerate = 10,
                      output = paste0("hive_videos/bee_video_random_eat_D",
                                 gsub("\\.","_",
                                 paste0(N_DAYS,
