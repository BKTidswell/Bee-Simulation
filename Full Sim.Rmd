...
To do:
 - Add ability for bees to eat
 - Have bees add honey or pollen to already existing or empty
...
hive <- array(EMPTY, dim = c(MAX_ROWS,MAX_COLS, 2)) #matrix(EMPTY,MAX_ROWS,MAX_COLS)
```
...
hive_matrix_to_graph <- function(hive_data){
  hive_df <- as.data.frame(as.table(hive_data[,,1]))
...
Bee <- R6Class("Bee",
               public = list(
                 ...
                 alter_comb = function(current_hive){
                   current_hive[self$cur_X,self$cur_Y,1] <- self$carry
                   return(current_hive)
                 },
                 buzz = function(){
