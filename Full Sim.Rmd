...
hive_matrix_to_graph <- function(hive_data, bees, nurses){
  ...
  g <- ggplot() + 
        ...
        geom_point(new_nurse_locs, mapping = aes(x,y), color = "black", size = 5) +
        geom_point(new_nurse_locs, mapping = aes(x,y), color = "orange", size = 3) +
...
make_random_hive <- function(){
  layer1 <- sample(c(EMPTY,BROOD,HONEY,POLLEN), MAX_ROWS*MAX_COLS, replace = TRUE, prob = c(0,0,1,1))
...
hive[8:13,8:13,1] <- BROOD
hive[8:13,8:13,2] <- sample(1:MAX_BROOD,1)
#hive[1,2,1] <- POLLEN
...
for(h in 1:24){
  ...
  for(m in 1:6){
    ...
    ggsave(paste0("hive_plots/hiveplot_",h,"_",m,".png"),hive_matrix_to_graph(hive,bees,nurses))
  }
