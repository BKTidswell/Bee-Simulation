...
hex_center_xs_df <- melt(cbind(id = 1:length(hex_center_xs), as.data.frame(hex_center_xs)), id.vars = "id", value.name = "x")
hex_center_ys_df <- melt(cbind(id = 1:length(hex_center_ys), as.data.frame(hex_center_ys)), id.vars = "id", value.name = "y")

hexdat_centers <- full_join(hex_center_xs_df, hex_center_ys_df)
...
hive_matrix_to_graph <- function(hive_data){
  hive_df <- as.data.frame(as.table(hive)) %>% pivot_wider(names_from = Var3, values_from = Freq)
  ...
  g <- ggplot(new_hexdat, aes(x, y)) + 
        geom_polygon(aes(group = id, fill = named_content, alpha = amount), colour = "black") +
...
make_random_hive <- function(){
  layer1 <- sample(c(EMPTY,BROOD,HONEY,POLLEN), MAX_ROWS*MAX_COLS, replace = TRUE, prob = c(1,1,1,1))
...
for(h in 1:24){
  ...
  for(m in 1:6){
    ...
    ggsave(paste0("hive_plots/hiveplot_",h,"_",m,".png"),hive_matrix_to_graph(hive))
  }
  #Then reset bees
  for(i in 1:length(bees)){
